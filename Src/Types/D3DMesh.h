...
enum GFXPlatformFormat {
	...
	eGFXPlatformFormat_U16 = 0x10,//unsigned shorts
	...
	eGFXPlatformFormat_U8x4 = 0x20,//unsigned int8 x4
	eGFXPlatformFormat_SN8 = 0x21,//signed int8
	eGFXPlatformFormat_UN8 = 0x22,//unsigned int8
	eGFXPlatformFormat_SN8x2 = 0x23,//signed int8 pair
	eGFXPlatformFormat_UN8x2 = 0x24,//unsigned int8 pair
	eGFXPlatformFormat_SN8x4 = 0x25,//S = signed, U = unsigned ,N = number i think. makes sense most normals/tangent use these, vec4s
	eGFXPlatformFormat_UN8x4 = 0x26,//unsigned int32
	...
	static MetaOpResult SerializeAsyncInternalResources(MetaStream* meta, D3DMesh* mesh) {
		...
		if (meta->IsRead()) {
			...
			for (int i = 0; i < res; i++) {
				...
				mesh->mInternalResources.AddElement(0, NULL, &handle);
				meta->EndBlock();
			}
		}
		else {
