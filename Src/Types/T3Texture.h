...
struct T3SamplerStateBlock {
	...
	inline static T3SamplerStateBlock::SamplerStateEntry smEntries[6];

	static void Initialize() {
		if (sInitialized)return;
		smEntries[T3SamplerStateValue::eSamplerState_WrapU_Value].mShift = 0x0;//0x14102C988
		smEntries[T3SamplerStateValue::eSamplerState_WrapU_Value].mMask = 0xF;//0x14102C98C, 4 bits, so a nibble of data
		smEntries[T3SamplerStateValue::eSamplerState_WrapV_Value].mShift = 0x4;//0x14102C990
		smEntries[T3SamplerStateValue::eSamplerState_WrapV_Value].mMask = 0xF0;//0x14102C994, 4 bits, so a nibble of data
		smEntries[T3SamplerStateValue::eSamplerState_Filtered_Value].mShift = 0x8;//0x14102C998
		smEntries[T3SamplerStateValue::eSamplerState_Filtered_Value].mMask = 0x100;//0x14102C99C //1 bit, so a bool
		smEntries[T3SamplerStateValue::eSamplerState_BorderColor_Value].mShift = 0x9;//0x14102C9A0
		smEntries[T3SamplerStateValue::eSamplerState_BorderColor_Value].mMask = 0x1E00;//0x14102C9A4, 4 bits, so a nibble of data
		smEntries[T3SamplerStateValue::eSamplerState_GammaCorrect_Value].mShift = 0xD;//0x14102C9A8
		smEntries[T3SamplerStateValue::eSamplerState_GammaCorrect_Value].mMask = 0x2000;//0x14102C9AC, 1 bit so a bool
		smEntries[T3SamplerStateValue::eSamplerState_MipBias_Value].mShift = 0xE;//0x14102C9B0
		smEntries[T3SamplerStateValue::eSamplerState_MipBias_Value].mMask = 0x3FC000;//0x14102C9B4, 8 bits, so a byte of data
		kDefault.mData = (((kDefault.mData & 0xFFFFFFF0 | 1) 
			& 0xFFFFFF0F | 0x10) & 0xFFFFFEFF | 0x100) & 0xFFC001FF;
		sInitialized = true;
	}
		...
		if (result == eMetaOp_Succeed) {
			...
			if (stream->mMode == MetaStreamMode::eMetaStream_Read) {
				...
				for (int i = 0; i < tex->mRegionHeaders.GetSize(); i++) {
					...
					header->mpAsyncStream->Copy(stream->mSection[(int)MetaStream::SectionType::eSection_Async].mpStream
					, stream->GetPos(), 0, header->mDataSize);
				}
				stream->EndAsyncSection();
			}
		}
